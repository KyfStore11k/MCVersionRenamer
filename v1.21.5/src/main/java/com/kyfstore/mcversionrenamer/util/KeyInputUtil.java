...
public class KeyInputUtil {
    ...
    public void handleKeyPress(Runnable toggleAction) {
        ...
        boolean isPressed = InputConstants.isKeyDown(Minecraft.getInstance().getWindow().getWindow(), keyCode);

        wasJustToggled = false;

        if (isPressed && !wasPressed) {
            wasPressed = true;
            if (!(Minecraft.getInstance().screen instanceof MCVersionRenamerScreen)) {
                toggleAction.run();
                wasJustToggled = true;
            }
        } else if (!isPressed) {
